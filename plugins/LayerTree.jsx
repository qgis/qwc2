...
class LayerTree extends React.Component {
    ...
    renderLayer = (layer, sublayer, path, enabled=true, inMutuallyExclusiveGroup=false, skipExpanderPlaceholder=false) => {
        ...
        if(this.props.showLegendIcons) {
            if(layer.legendUrl) {
                let request = layer.legendUrl + (layer.legendUrl.indexOf('?') === -1 ? '?' : '&') + "SERVICE=WMS&REQUEST=GetLegendGraphic&VERSION=" + (layer.version || "1.3.0") + "&FORMAT=image/png&LAYER=" + encodeURIComponent(sublayer.name) + "&SCALE=" + Math.round(this.props.mapScale);
        ...
        body += this.props.layers.map(layer => {
            if(!layer.visibility) {
                ...
                return layer.params.LAYERS ? layer.params.LAYERS.split(",").reverse().map(sublayer => {
                    let request = layer.legendUrl + (layer.legendUrl.indexOf('?') === -1 ? '?' : '&') + "SERVICE=WMS&REQUEST=GetLegendGraphic&VERSION=" + (layer.version || "1.3.0") + "&FORMAT=image/png&LAYER=" + sublayer + "&SCALE=" + Math.round(this.props.mapScale);
